...
import models.game_object.GameObject;
import models.game_object.Player;
...
public class InvadersGameState {
    ...
    private final models.Polygon default_enemy_model = Polygon.circle(default_enemy_radius, 10);
    ...
    public void updateAll() {
        ...
        if (this.canvas.isKeyPressed(KeyEvent.VK_A)) playerAcc = playerAcc.add(new Vector(-default_player_acc, 0));
        if (this.canvas.isKeyPressed(KeyEvent.VK_D)) playerAcc = playerAcc.add(new Vector(default_player_acc, 0));
        if (!this.canvas.isKeyPressed(KeyEvent.VK_D) && !this.canvas.isKeyPressed(KeyEvent.VK_A) && this.player.getVelocity().magnitude() != 0)
            playerAcc = this.player.getVelocity().unit().times(-default_player_friction);
        if (this.canvas.isKeyPressed(KeyEvent.VK_LEFT)) playerTurn += default_player_turn_rate;
        if (this.canvas.isKeyPressed(KeyEvent.VK_RIGHT)) playerTurn -= default_player_turn_rate;
    ...
    private void checkIntersections() {
        ...
        for (GameObject enemy : this.enemies) {
            for (GameObject projectile : this.projectiles) {
            ...
            if (enemy.getPosition().minus(this.player.getPosition()).magnitude() < 10 || enemy.getPosition().getComponent(1) < -default_scale)
                this.over = true;
        }
    ...
    public void endScreen() {
        ...
        if (this.win) text = "YOU WIN, R TO RESTART OR Q TO QUIT";
    ...
    public boolean mustRestart() {
        return this.canvas.isKeyPressed(KeyEvent.VK_R);
    }
