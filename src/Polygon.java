...
public class Polygon {
	...
	public Polygon rotate(double theta) {
		...
		for (int i = 0 ; i < this.points; i++) pointsNew[i] = centroid.add(Vector2D.rotate(pointsOld[i].minus(centroid), theta));
		return new Polygon(pointsNew);
	}
	...
	public Polygon rotate(Vector reference, double theta) {
		...
		for (int i = 0 ; i < this.points; i++) pointsNew[i] = reference.add(Vector2D.rotate(pointsOld[i].minus(reference), theta));
		return new Polygon(pointsNew);
	}
	...
	public double getArea() {
		double inner = 0;
		for (int i = 0; i < this.points; i++) inner += ((this.x[i] * this.y[(i + 1) % this.points]) - (this.x[(i + 1) % this.points] * this.y[i]));
		return 0.5 * inner;
	}
